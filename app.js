// GHL Custom App Integration for Meta Audience Sync
let currentLocationId = '';
let apiToken = '';
let contactsData = [];

// Updated FIELD_MAPPING based on your actual GHL fields
const FIELD_MAPPING = {
    // Primary Meta Audience Fields
    hashed_email: 'primary_sha256_personal',  // or 'primary_sha256_business'
    hashed_phone: 'primary_mobile_phone',      // or 'primary_direct_number' or 'primary_personal_number'
    
    // Additional demographic fields for Meta Custom Audiences
    ...
    // Additional targeting fields from your GHL data
    gender: 'gender',
    age_range: 'age_range',
    exact_age: 'exact_age',
...
// Updated field detection to match your GHL structure
function getFieldSample(contacts, fieldName) {
    for (const contact of contacts) {
        // Check custom fields array - this is where your data lives
        if (contact.customFields && Array.isArray(contact.customFields)) {
            // Look for field by fieldKey (as shown in your JSON)
            const field = contact.customFields.find(f => 
                f.fieldKey === `contact.${fieldName}` || 
        ...
        // Also check for standard contact properties
...
// Updated contact field getter for your GHL structure
function getContactField(contact, fieldName) {
    // Check custom fields array (your main data source)
    if (contact.customFields && Array.isArray(contact.customFields)) {
        // Try multiple matching strategies based on your JSON structure
        const field = contact.customFields.find(f => 
            f.fieldKey === `contact.${fieldName}` || 
            f.fieldKey === fieldName ||
            f.name === fieldName ||
    ...
    // Fallback to standard contact properties
    if (contact[fieldName] !== undefined && contact[fieldName] !== null) {
...
// Updated displayFieldPreview with better field matching
function displayFieldPreview(contacts) {
    ...
    for (const [metaField, ghlField] of Object.entries(FIELD_MAPPING)) {
        ...
        html += `<span class="badge ${hasField ? 'bg-success' : 'bg-danger'}">${hasField ? '✓ Found' : '✗ Missing'}</span>`;
        html += `</td>`;
        html += `<td style="padding: 10px; border: 1px solid #ddd; font-family: monospace; font-size: 12px;">`;
        if (fieldData.sample) {
            html += `${String(fieldData.sample).substring(0, 30)}${String(fieldData.sample).length > 30 ? '...' : ''}`;
        } else {
            html += '<em class="text-muted">No data</em>';
        }
        html += `</td>`;
        html += '</tr>';
    }
    ...
    // Update counters and enable export
    const exportBtn = document.getElementById('exportBtn');
    if (exportBtn) {
...
// Transform contacts to Meta format using your GHL field structure
function transformToMetaFormat(contacts) {
    return contacts
        .filter(contact => {
            // Must have at least email or phone for Meta audience
            const emailField = getContactField(contact, 'primary_sha256_personal') || 
                               getContactField(contact, 'primary_sha256_business') ||
                               getContactField(contact, 'hashed_email');
            const phoneField = getContactField(contact, 'primary_mobile_phone') || 
                               getContactField(contact, 'primary_direct_number') || 
                               getContactField(contact, 'primary_personal_number') ||
                               getContactField(contact, 'hashed_phone');
...
// Export function for Meta Audience
